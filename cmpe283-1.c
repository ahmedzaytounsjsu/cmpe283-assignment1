...
struct capability_info vmxExitCtls[11] =
{
...
struct capability_info vmxProcbased[21] =
{
...
struct capability_info vmxProcbasedCtlsSecondary[23] =
{
...
struct capability_info vmxEntryCtls[9] =
{
...
detect_vmx_features(void)
{
	...
	pr_info("Exit Controls MSR: 0x%llx\n",
		(uint64_t)(lo | (uint64_t)hi << 32));
	report_capability(vmxExitCtls, 11, lo, hi);

	/* VMX Probased Controls */ 
	rdmsr(IA32_VMX_PROCBASED_CTLS, lo, hi);
	pr_info("Procbased Controls MSR: 0x%llx\n",
		(uint64_t)(lo | (uint64_t)hi << 32));
	report_capability(vmxProcbased, 21, lo, hi);

	/* VMX Procbased Scondary Controls */ 
	rdmsr(IA32_VMX_PROCBASED_CTLS2, lo, hi);
	pr_info("Procbased secondary Controls MSR: 0x%llx\n",
		(uint64_t)(lo | (uint64_t)hi << 32));
	report_capability(vmxProcbasedCtlsSecondary, 23, lo, hi);

	/* VMX Entry Controls */ 
	rdmsr(IA32_VMX_ENTRY_CTLS, lo, hi);
	pr_info("Entry Controls MSR: 0x%llx\n",
		(uint64_t)(lo | (uint64_t)hi << 32));
	report_capability(vmxEntryCtls, 9, lo, hi);
